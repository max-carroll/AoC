const rawInput = `Game 1: 4 green, 7 blue; 2 blue, 4 red; 5 blue, 2 green, 2 red; 1 green, 3 red, 9 blue; 3 green, 9 blue; 7 green, 2 blue, 2 red
...
function getColourFromString(s: string, colour: string): number {
  const regex = new RegExp(`\\d+ ${colour}`);
  const match = s.match(regex);
  const result = parseInt(match?.[0] ?? "0", 10);
  return result;
}
...
function ParseData(): Array<Game> {
  ...
  const games = textLines.map((line: string) => getGameDataFromLine(line));

  return games;
}

function getGameDataFromLine(line: string) {
  const [stringContainingGameId, stringContainingGameData] = line.split(":");

  let id = 0;
  const gameIdMatch = stringContainingGameId.match(/\d+/);

  if (gameIdMatch) {
    id = parseInt(gameIdMatch[0]);
  }

  let subsets = stringContainingGameData
    .split(";")
...
function getSubsetFromString(subsetString: string): CubeInfo {
  const blue = getColourFromString(subsetString, "blue");
  const red = getColourFromString(subsetString, "red");
  const green = getColourFromString(subsetString, "green");
